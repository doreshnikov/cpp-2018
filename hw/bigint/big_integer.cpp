...
big_integer::big_integer() : negative(false), data{0} {}
...
big_integer::big_integer(unsigned int a) : negative(false), data{a} {}
...
big_integer operator-(const big_integer &lhs, const big_integer &rhs) {
    if (lhs.negative == rhs.negative) {
        ...
        bool carry = false;
        std::vector<atom> data(lhs.data);

        for (size_t index = 0; index < data.size(); index++) {
            overflow_atom sub = to_overflow(rhs.get_atom(index)) + carry;
            carry = sub > data[index];
            if (carry) {
                data[index] = to_atom((sub - data[index] - 1) ^ ATOM_MAX);
            } else {
                data[index] -= sub;
            }
        }
...
big_integer big_integer::from_bitwise() const {
    if (!negative) {
        ...
        std::vector<atom> inverted(data);
        for (atom &digit : inverted) {
            digit = ~digit;
        }
